# Anschreiben — zwei Varianten

> **Vor dem Abschicken:** `[Name]` durch den echten Namen ersetzen und unten deinen Namen
> eintragen. Terminvorschläge ggf. an deinen Kalender anpassen.

---

## Variante A — E-Mail (mit Deck & Kostenblatt im Anhang)

**Betreff:** Horrorgeticon Ops — das Ding aus dem WhatsApp-Chat von gestern 👻

Hi [Name],

danke nochmal für deine Reaktion gestern Abend — „klingt spannend, zeig mal mehr“ ist
exakt die Sorte Druck, die ich gebraucht habe. Hier ist also: mehr.

Im Anhang findest du zwei Dateien:

1. **Das Pitchdeck** (`Horrorgeticon_Ops_Pitchdeck.html`) — einfach im Browser öffnen,
   mit den Pfeiltasten durchblättern (oder am Handy wischen). Funktioniert offline,
   braucht keine Installation. Wichtigster Punkt vorweg: Alles, was darin nach
   Screenshot aussieht, **ist** ein Screenshot. Die Software existiert, läuft und
   wartet nur darauf, eure Horrornacht zu sortieren. Für die PowerPoint-Fraktion
   liegt das Ganze auch als `.pptx` bei.

2. **Die Kostenaufstellung** (`Kostenaufstellung.html`) — gesondert, damit das Deck
   Spaß machen darf und die Zahlen trotzdem seriös bleiben. Kurzfassung: Die
   Entwicklung ist passiert und kostet euch nichts. Das Saisonpaket (Einrichtung,
   Schulung, Generalprobe, Eventnacht-Support, Nachlese) liegt marktüblich bei
   ~12.800 € — ihr zahlt 3.400 €. Freundschaftspreis, mit Backfisch-Garantie.
   Kein Witz. Also doch, ein bisschen — aber ein vertraglich gemeinter.

Worum es im Kern geht, in einem Satz: **Die Horrornacht darf eskalieren, die Orga
nicht.** Leitstand fürs Management, Tablet für die Maze Leads, App für die Scare
Actors, QR-Marken fürs Catering — und Excel wird in Würde zum Importformat degradiert.
Neu: ein Kids Day Modus, der 120 Darsteller in einem Rutsch auf familienfreundlich
umschaltet.

Mein Vorschlag: **30 Minuten, ich bringe das Live-System mit, ihr bringt die Skepsis.**
Ich kann Anfang nächster Woche fast immer — Montag- oder Dienstagabend wäre ideal,
aber ich richte mich nach euch. Wenn ihr mögt, gleich da, wo die Nacht stattfinden
soll, dann sehen wir das Gelände direkt mit den Augen der Live-Karte.

Wann passt's dir?

Beste Grüße & bis gleich im Kalender,
[Dein Name]

P.S.: Die Demo läuft auf meinem Laptop, braucht kein Internet und überlebt laut
Testsuite sogar einen Absturz mitten in der Nacht. Ich habe es 300-fach prüfen
lassen. Man will ja nicht unvorbereitet zu Leuten, die beruflich Menschen erschrecken.

---

## Variante B — WhatsApp (kurz, als Anstoß direkt nach der Mail)

> Moin [Name]! Gestern war nicht nur Bier-Idee 😄 Hab dir gerade das komplette
> Paket gemailt: Pitchdeck mit echten Screenshots (die Software läuft wirklich
> schon!) plus Kostenblatt — Spoiler: Freundschaftspreis. Schau's dir in Ruhe an,
> aber sag mir am besten gleich einen Termin für 30 min Live-Demo nächste Woche.
> Ich bring den Laptop, ihr bringt die Skepsis. 👻🐟

---

## Spickzettel für den Termin selbst (nur für dich)

- **Einstieg (2 min):** Hook aus Slide 2 erzählen — die 23:40-Uhr-Szene. Lacher abholen, dann ernst werden.
- **Demo statt Folien (15 min):** Deck nur bis Slide 4, dann ins Live-System wechseln.
  Reihenfolge: Leitstand-Dashboard → Live-Karte → einmal Pause freigeben → Actor-Handy zeigen → QR-Marke scannen lassen (Publikumsbeteiligung!).
- **Einwände, die kommen werden:**
  - *„Was, wenn das WLAN ausfällt?"* → Läuft im lokalen LAN auf einem Laptop, kein Internet nötig. Heartbeat zeigt Funklöcher sogar an.
  - *„Unsere Crew ist nicht technisch."* → Login ist Code + PIN, die Actor-App hat vier Knöpfe. Schulung + Spickzettel sind im Paket.
  - *„Was kostet das wirklich?"* → Kostenblatt rüberschieben. Die Zahlen stehen, nichts kommt obendrauf.
- **Abschluss:** Nicht „meldet euch mal", sondern konkret: „Sollen wir die Generalprobe
  für September gleich grob terminieren?" — wer einen zweiten Termin hat, hat ein Projekt.
